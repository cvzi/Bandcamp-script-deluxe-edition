...
function addAllAlbumsAsHeadings () {
  ...
  for (let i = 0; i < as.length; i++) {
    ...
    if (!isAlreadyInPlaylist(url)) {
      let title = url.replace('https://', '').replace('http://', '').replace(BANDCAMPDOMAIN, '')
        .replace('bandcamp.com', '').replace('bandcamp.', '').replace('www.', '').replace(/\/$/, '') // This is just a fallback title
